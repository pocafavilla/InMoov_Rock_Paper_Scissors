...
class MakeDataset():
    ...
    def read(self,subdir):
        img_type = '.jpg'
        dir = os.path.join('/localdata/Leona_Maehler/practice/Inmv/t2/data/', subdir)
        imgs = []
    ...
    def read_all(self, end):
        ...
        for i, subdir in enumerate(os.listdir("/localdata/Leona_Maehler/practice/Inmv/t2/data/")):
            if i>= end:
                for e in range(len(all_imgs)):
                    all_imgs[e] = np.asarray(all_imgs[e])
                return all_imgs
            for j, subsubdir in enumerate(['c0','c1','c2']):           
                if os.path.isdir(os.path.join('/localdata/Leona_Maehler/practice/Inmv/t2/data/', 
